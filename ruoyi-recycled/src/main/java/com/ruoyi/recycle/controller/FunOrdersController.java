...
//    @RequiresPermissions("recycle:orders:add")
    ...
    public AjaxResult addSave(@Validated FunOrders funOrders, BindingResult result) {
        ...
        if (orderStatus.equals("未发货") || orderStatus.equals("待支付")) {
            ...
            if (status.equals(AliPayApiConfig.TRADE_SUCCESS)) {
                funOrders.setZfPrice(Double.parseDouble(response.getBuyerPayAmount()));
                funOrders.setOrdersStatus("未发货");
                //TODO 发送商品API
            } else if (status.equals(AliPayApiConfig.WAIT_BUYER_PAY)) {
        ...
        return toAjax(funOrdersService.insertFunOrders(funOrders));
    }
    ...
    public AjaxResult editSave(FunOrders funOrders) {
        ...
        if (funOrders.getOrdersStatus().equals("退款完成")) {
