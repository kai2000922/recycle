...
public class ExcelUtil<T> {
    ...
    public void init(List<T> list, String sheetName, String title, Type type) {
        ...
        if (this.customFlag) {
            int count = 0;
            // 根据换行符计算行高
            if (this.title.contains(System.lineSeparator())) {
                count = this.title.split(System.lineSeparator()).length - 1;
            }
            createCustomTitle(count);
        } else {
            createTitle();
        }
    }
     ...
     * 创建excel第一行标题(自定义)
     */
    public void createCustomTitle(int height) {
        if (StringUtils.isNotEmpty(title)) {
            Row titleRow = sheet.createRow(rownum == 0 ? rownum++ : 0);
            // 计算行高
            titleRow.setHeightInPoints(30 * (1 + height));
            Cell titleCell = titleRow.createCell(0);
            // 自定义数据不需要表头样式
            // titleCell.setCellStyle(styles.get("title"));
            titleCell.setCellValue(title);
     ...
     * @return 结果
     */
    public AjaxResult exportExcel(List<T> list, String sheetName, String title, boolean custom) {
